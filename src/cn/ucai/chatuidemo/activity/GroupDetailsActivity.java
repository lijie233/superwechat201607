...
import cn.ucai.chatuidemo.task.DownloadMemberMapListTask;
		...
		utils.setRequestUrl(I.REQUEST_ADD_GROUP_MEMBERS)
				...
				.execute(new OkHttpUtils2.OnCompleteListener<String>() {
					...
					public void onSuccess(String s) {
						...
						GroupAvatar groupAvatar = (GroupAvatar) result.getRetData();
						if (result != null && result.isRetMsg()) {
							new DownloadMemberMapListTask(getApplicationContext(),groupId).execute();
							runOnUiThread(new Runnable() {
								public void run() {
									progressDialog.dismiss();
									setResult(RESULT_OK);
...
//				holder.textView.setText(username);
	...
	private void setUpdateMemverListener() {
	...
	private void deleteMembersFromAppGroup(final String username,final boolean isExit) {
		...
		if (group != null) {
			...
			utils.setRequestUrl(I.REQUEST_DELETE_GROUP_MEMBER)
					.addParam(I.Member.GROUP_ID,String.valueOf(group.getMAvatarId()))
